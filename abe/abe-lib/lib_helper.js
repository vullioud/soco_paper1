...
*    @method mergeOptions
*/
lib.mergeOptionsSafe = function(defaults, options) {
  const merged = {};
  for (const key in defaults) {
...
lib.mergeOptions = lib.mergeOptionsSafe;
console.log("mergeOptionsSafe defined and aliased to mergeOptions");
/**
...
lib.createSTP = function(stp_name, ...concepts) {
    ...
    for (const concept of concepts) {
        ...
        // Case 1: The concept is a single, valid activity object
        if (concept.hasOwnProperty('type')) {
            program[`activity${activityCounter}`] = concept;
            activityCounter++;
            continue;
        }
        
        // Case 2: The concept is a "program" object containing multiple activities and other properties
        for (const key in concept) {
            const prop = concept[key];
            
            // A valid activity is an object and MUST have a 'type' property.
            if (typeof prop === 'object' && prop !== null && prop.hasOwnProperty('type')) {
                // Use the key from the program object if it's descriptive
                const newKey = program.hasOwnProperty(key) ? `${key}_${activityCounter}` : key;
                program[newKey] = prop;
            } else if (typeof prop !== 'object' || !prop.hasOwnProperty('type')) {
                // --- NEW --- This handles properties like 'description'
                // Avoid overwriting existing properties unless they are functions (like events)
