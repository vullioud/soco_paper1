...
lib.thinning.fromBelow = function(options) {
    ...
    const defaultOptions = {
		...
		schedule: {min: 30, opt: 40, max: 50},
		interval: 5,
		times: 5,
		block: true,
		mode: 'simple',
        thinningShare: 0.1,
		thinningClasses: [80, 15, 4, 0.9, 0.1],
    ...
    // Capture the default value in a constant to create a stable closure.
    const defaultThinningShare = opts.thinningShare;
    const activityTargetValue = (opts.mode === 'dynamic')
        ? function() { return (stand.flag('thinningShare') || defaultThinningShare) * 100; }
        : opts.thinningShare * 100;
    // --- END MODIFIED LOGIC ---
	...
	program["thinning"] = {
        ...
        thinning: 'custom',
        targetValue: activityTargetValue, // Use the robust function here
		targetVariable: opts.ranking, 
		targetRelative: true,
		filter: undefined, // treeFilter is defined too late, handle inside onEvaluate if needed
        minDbh: 0,
		...
		onExecuted: function() {
			lib.dbg(`Thinning from below executed.`);
		},
	};
