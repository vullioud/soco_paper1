...
lib.thinning.selectiveThinning = function(options) {
    ...
    // Instead of creating functions that depend on the 'opts' variable via a closure,
    // we create self-contained functions that have the default values baked in.
    
    const defaultN = opts.nTrees; // Capture the default value
    const activity_N = (opts.mode === 'dynamic') 
        ? function() { return stand.flag('nTrees') || defaultN; } // Use captured default
        : opts.nTrees;

    const defaultNComp = opts.nCompetitors; // Capture the default value
    const activity_NCompetitors = (opts.mode === 'dynamic')
        ? function() { 
            const ageFactor = Math.max(Math.min(1.0, -0.01 * stand.age + 1.2), 0.0);
            const baseCompetitors = stand.flag('nCompetitors') || defaultNComp; // Use captured default
            return Math.max(baseCompetitors * ageFactor, 1);
          }
        : opts.nCompetitors;
    // --- END MODIFIED LOGIC ---
    ...
    program["Selective_selector"] = {
        ...
        thinning: 'selection',
        N: activity_N, // Use the robust function
        NCompetitors: activity_NCompetitors, // Use the robust function
        ...
        description: `Part of selective thinning - mark crop trees and competitors.`
    };
    ...
    program["Selective_remover"] = {
        ...
        action: function() {
            ...
            stand.obj.lib.selective_thinning_counter++;
        ...
        description: `Part of selective thinning - remove selected competitors.`
    };
