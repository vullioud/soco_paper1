...
DATA_IN <- file.path(OUT_ROOT, "data", "outbreak_harvest.csv")
...
unlink(file.path(MODELS_OUT, "outbreak_harvest_*.rds"))
unlink(file.path(TABLES_OUT, "outbreak_harvest_*.csv"))
unlink(file.path(FIGURES_OUT, "outbreak_harvest_*.png"))

message("=== Outbreak harvest analysis ===")
message("This script compares outbreak harvest responses against paired controls.")

outbreak_harvest <- fread(DATA_IN)
outbreak_harvest[, landscape := stats::relevel(factor(landscape), ref = "CL02")]
outbreak_harvest[, aggregation := stats::relevel(factor(aggregation), ref = "matched_High")]
outbreak_harvest[, init_uid := factor(init_uid)]

response_map <- data.table(
  response = c(
    "delta_planned_pre_mean",
    "delta_planned_post_mean",
    "did_planned_mean",
    "delta_salvage_pulse_mean",
    "delta_salvage_post_mean"
  ),
  title = c(
    "Outbreak planned harvest: pre-period treatment-control gap",
    "Outbreak planned harvest: post-period treatment-control gap",
    "Outbreak planned harvest: change relative to control",
    "Outbreak salvage: pulse-period treatment-control gap",
    "Outbreak salvage: recovery-period treatment-control gap"
  ),
  ylab = c(
    "Planned harvest gap (100-149)",
    "Planned harvest gap (161-250)",
    "Planned harvest difference-in-differences",
    "Salvage gap (150-160)",
    "Salvage gap (161-250)"
  ),
  stem = c(
...
for (i in seq_len(nrow(response_map))) {
  ...
  message(sprintf("Fitting outbreak harvest model set: %s", response))
  dat <- outbreak_harvest[is.finite(get(response))]
  ...
  saveRDS(
    list(
      ...
      analysis = "outbreak_harvest"
    ),
    file.path(MODELS_OUT, sprintf("outbreak_harvest_%s.rds", stem))
  )
  ...
  anova_dt[, `:=`(analysis = "outbreak_harvest", response = response)]
  ...
  summary_dt[, `:=`(analysis = "outbreak_harvest", response = response)]
  ...
  for (landscape_name in unique(as.character(dat$landscape))) {
    ...
    tuk[, `:=`(analysis = "outbreak_harvest", response = response, landscape = landscape_name)]
    pairwise_tukey <- rbind(pairwise_tukey, tuk, fill = TRUE)
  }

  raw_plot <- ggplot(dat, aes(x = aggregation, y = get(response))) +
    geom_boxplot(outlier.shape = NA, alpha = 0.3, fill = "#dde6ef", color = "#355c7d") +
    geom_jitter(width = 0.15, alpha = 0.6, size = 1.6, color = "#355c7d") +
    facet_wrap(~ landscape, scales = "free_y") +
    labs(title = plot_title, x = "Aggregation", y = ylab) +
    theme_minimal(base_size = 11) +
    theme(axis.text.x = element_text(angle = 30, hjust = 1))

  ggsave(file.path(FIGURES_OUT, sprintf("outbreak_harvest_%s_raw.png", stem)), raw_plot, width = 12, height = 7, dpi = 300)

  summary_plot <- ggplot(summary_dt, aes(x = aggregation, y = mean)) +
    geom_point(size = 2.4, color = "#355c7d") +
    geom_errorbar(aes(ymin = ci_low, ymax = ci_high), width = 0.18, color = "#355c7d") +
    facet_wrap(~ landscape, scales = "free_y") +
    labs(title = plot_title, subtitle = "Mean and 95% interval by landscape", x = "Aggregation", y = ylab) +
    theme_minimal(base_size = 11) +
    theme(axis.text.x = element_text(angle = 30, hjust = 1))

  ggsave(file.path(FIGURES_OUT, sprintf("outbreak_harvest_%s_summary.png", stem)), summary_plot, width = 12, height = 7, dpi = 300)
}
...
parameter_ci <- extract_selected_model_parameter_ci(
  ...
  analysis_name = "outbreak_harvest"
)

fwrite(model_comparison, file.path(TABLES_OUT, "outbreak_harvest_model_comparison.csv"))
fwrite(anova_comparison, file.path(TABLES_OUT, "outbreak_harvest_anova_comparison.csv"))
fwrite(descriptive_summary, file.path(TABLES_OUT, "outbreak_harvest_descriptive_summary.csv"))
fwrite(pairwise_tukey, file.path(TABLES_OUT, "outbreak_harvest_pairwise_tukey.csv"))
fwrite(parameter_ci, file.path(TABLES_OUT, "outbreak_harvest_selected_model_parameter_ci.csv"))

message("Saved outbreak harvest selected-model parameter CI table")
message("=== Outbreak harvest analysis complete ===")
